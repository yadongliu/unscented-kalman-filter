...
UKF::UKF() {
  ...
  */
  ...
  n_aug_ = 7;
  lambda_ = 3 - n_aug_;
  n_z_ = 3; // 3 for radar, 2 for lidar
...
void UKF::ProcessMeasurement(MeasurementPackage meas_package) {
  ...
  */
  ...
  if (meas_package.sensor_type_ == MeasurementPackage::RADAR && use_radar_) {
    ...
    cout << "radar nis: " << NIS_radar_ << std::endl;
  } else if (meas_package.sensor_type_ == MeasurementPackage::LASER && use_laser_) {
    n_z_ = 2;
    UpdateLidar(meas_package);
    cout << "lidar nis: " << NIS_radar_ << std::endl;
  }
...
void UKF::Prediction(double delta_t) {
  ...
  */
  ...
  SigmaPointPrediction(Xsig_out, &Xsig_pred_, delta_t);
...
void UKF::SigmaPointPrediction(MatrixXd Xsig_aug, MatrixXd* Xsig_out, float delta_t) {
...
void UKF::PredictRadarMeasurement(VectorXd* z_out, MatrixXd* S_out, MatrixXd* Zsig_out) {
  ...
  //radar measurement noise standard deviation radius in m
  double std_radr = std_radr_; // 0.3

  //radar measurement noise standard deviation angle in rad
  double std_radphi =  std_radphi_; // 0.0175;

  //radar measurement noise standard deviation radius change in m/s
  double std_radrd = std_radrd_; // 0.1;
...
void UKF::UpdateRadar(MeasurementPackage meas_package) {
  ...
  */
  int n_z = 3;
  // measurement prediction z_pred and covariance matrix S
  ...
  //create example vector for incoming radar measurement (rho in m, phi in rad, rho_dot in m/s)
...
void UKF::UpdateLidar(MeasurementPackage meas_package) {
  ...
  You'll also need to calculate the lidar NIS (normalized innovatin squared)
  */
  int n_z = 2;
  // measurement prediction z_pred and covariance matrix S
  VectorXd z_pred = VectorXd(n_z);
